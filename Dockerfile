...
RUN sed -i -e "s|conf/game.conf|src/github.com/yasshi2525/RushHour/conf/game.conf|" src/github.com/yasshi2525/RushHour/app/services/config.go && \
    ...
    sed -i -e "s|^baseurl = .*$|baseurl = \"baseurl = ${baseurl}\"|" conf/secret.conf && \
    sed -i -e "s/__SALT__/\"${salt}\"/" conf/secret.conf && \
    sed -i -e "s/______KEY_______/\"${key}\"/" conf/secret.conf && \
    sed -i -e "s/__STATE__/\"${state}\"/" conf/secret.conf && \
    sed -i -e "s/__TWITTER_TOKEN__/\"${twitter_token}\"/" conf/secret.conf && \
    sed -i -e "s/__TWITTER_SECRET__/\"${twitter_secret}\"/" conf/secret.conf && \
    sed -i -e "s/__GOOGLE_CLIENT__/\"${google_client}\"/" conf/secret.conf && \
    sed -i -e "s/__GOOGLE_SECRET__/\"${google_secret}\"/" conf/secret.conf
