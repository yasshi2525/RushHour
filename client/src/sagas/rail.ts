...
export function* generateDepart(
  action: ReturnType<typeof Action.depart.request>
) {
  return yield generateRequest(
    () => postDepart(action.payload),
    action,
    Action.depart
  );
}
...
export function* generateExtend(
  action: ReturnType<typeof Action.extend.request>
) {
  return yield generateRequest(
    () => postExtend(action.payload),
    action,
    Action.extend
  );
}

export function* generateConnect(
  action: ReturnType<typeof Action.connect.request>
) {
  return yield generateRequest(
    () => postConnect(action.payload),
    action,
    Action.connect
  );
}
