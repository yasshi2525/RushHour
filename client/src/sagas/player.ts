...
async function editSettings(entry: Entry) {
  let res = await http(`${settingsURL}/${entry.key}`, Method.POST, {
    value: entry.value
  });
...
export function* generatePlayers(
  action: ReturnType<typeof Action.players.request>
) {
  return yield generateRequest(
    () => fetchPlayers(action.payload.model.gamemap),
    action,
    Action.players
  );
}

export function* generatePlayersPlain(
  action: ReturnType<typeof Action.playersPlain.request>
) {
  return yield generateRequest(
    () => fetchPlayers(),
    action,
...
export function* generateLogin(
  action: ReturnType<typeof Action.login.request>
) {
  return yield generateRequest(
...
export function* generateSignOut(
  action: ReturnType<typeof Action.signout.request>
) {
  return yield generateRequest(
...
export function* generateSettings(
  action: ReturnType<typeof Action.settings.request>
) {
  return yield generateRequest(() => fetchSettings(), action, Action.settings);
}

export function* generateEditSettings(
  action: ReturnType<typeof Action.editSettings.request>
) {
  return yield generateRequest(
