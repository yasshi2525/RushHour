...
import Anchor from "common/models/anchor";
import Cursor from "common/models/cursor";
import { RailNode } from "common/models/rail";
...
export abstract class CursorHandler<T> {
  ...
  protected requestZoom(client: Point) {
    let center = getZoomPos(
      this.model,
      { x: this.model.coord.cx, y: this.model.coord.cy },
      this.model.coord.scale,
      client,
      -1
    );
  ...
  protected handle(client: Point) {
    ...
    switch (this.model.menu) {
        ...
        if (this.cursor.selected === undefined) {
          this.dispatch(
            depart.request({
              ...
              y: server.y,
              scale: Math.floor(
                this.model.coord.scale - this.model.delegate + 1
              )
            })
          );
        } else {
      ...
      case MenuStatus.EXTEND_RAIL:
        if (this.cursor.selected === undefined) {
          if (
            this.anchor.object !== undefined &&
            this.cursor.get("activation")
          ) {
            this.dispatch(
              extend.request({
                model: this.model,
