...
import { AnimatedSpriteProperty } from "common/interfaces/pixi";
...
export default class extends AnimatedSpriteModel implements Monitorable {
  ...
  constructor(
    options: AnimatedSpriteProperty & {
      offset: number;
      anchor: Anchor;
      destroyer: Destroyer;
    }
  ...
  protected getObjectOnChunk(except: Monitorable | undefined = undefined) {
    ...
    switch (this.props.menu) {
      ...
      case MenuStatus.EXTEND_RAIL:
        selected = this.model.gamemap.getOnChunk(
          "rail_nodes",
          this.props.pos,
          this.model.myid
        );
    ...
    return selected === except
      ? undefined
      : (selected as PointModel | undefined);
  }
  protected activate(objOnChunk: Monitorable | undefined) {
    ...
    switch (this.props.menu) {
      case MenuStatus.EXTEND_RAIL:
        ...
        if (
          this.anchor.object instanceof RailNode &&
          this.selected instanceof RailNode
        ) {
          let anchor = this.anchor.object;
          let selected = this.selected;
          let link = Object.keys(anchor.out).find(
            eid => eid != "cursorIn" && anchor.out[eid].to == selected
          );
          if (link !== undefined) {
            activation = false;
          }
  ...
  protected getTint() {
    return this.props.activation
      ? graphicsOpts.tint.info
      : graphicsOpts.tint.error;
  }
  ...
  genAnchorStatus() {
    if (this.selected === undefined) {
      ...
      let res = {
        pos: this.selected.get("pos"),
        type: "",
        cid: this.selected.get("cid")
      };
  ...
  protected isVisible() {
    return (
      this.props.menu === MenuStatus.SEEK_DEPARTURE ||
      this.props.menu === MenuStatus.EXTEND_RAIL
    );
  }
}
