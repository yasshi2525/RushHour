...
import { Coordinates, config } from "common/interfaces/gamemap";
import { GameModelProperty } from "common/interfaces/pixi";
...
export default class {
  ...
  protected getDelegate() {
    if (this.renderer.width < 600) {
      // sm
      return 2;
    } else if (this.renderer.width < 960) {
      // md
      return 3;
    } else if (this.renderer.width < 1280) {
      // lg
      return 3;
    } else {
      // xl
  ...
  protected setCenter(x: number, y: number) {
    ...
    let shortRadius = Math.pow(
      2,
      this.coord.scale - 1 + Math.log2(short / long)
    );
